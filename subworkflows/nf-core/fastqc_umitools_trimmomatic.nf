...
include { TRIMMOMATIC       } from '../../modules/nf-core/modules/trimmomatic/main'       addParams( options: params.trimmomatic_options )

workflow FASTQC_UMITOOLS_TRIMMOMATIC {
    take:
    ...
    trimmomatic_version = Channel.empty()
    if (!skip_trimming) {
        TRIMMOMATIC ( umi_reads ).reads.set { trim_reads }
        //trim_html  = TRIMGALORE.out.html
        //trim_zip   = TRIMGALORE.out.zip
        //trim_log   = TRIMGALORE.out.log
        trimmomatic_version = TRIMMOMATIC.out.version
    }
    ...
    trimmomatic_version //    path: *.version.txt
}
